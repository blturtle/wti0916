...
app = Flask(__name__)
db = pymysql.connect(host='localhost', port=3306, user='root', passwd='1234', db='wti', charset="utf8")
cursor = db.cursor()
...
def get_wti_data():
    sql = "select Date, WTI_price, WTI_id from wti_table where (mod(WTI_id, 7)=0) and (Date between '2022-01-01' and '2022-07-31')"
    cursor.execute(sql)
    results = cursor.fetchall()  # ((2022-07-01, '유가1', 아이디1), (2022-07-02, '유가2', 아이디2))
    wti_data = []
...
# @app.route("/query_test/", methods=["GET"])  ## 예시 코드 (참고용)
# def query_test():
...
def chart_addIndex(period):
    ...
    td = datetime.timedelta(days=server_per)
...
def get_news_detail(newsId):
    ...
    for result in results:
        news.append({
            'news_id': result[0],
            'news_title': result[1],
            'news_short_desc': result[2],
            'news_content': result[3],
            'news_author': result[4],
            'news_publisher': result[5],
            'news_date': result[6]
        })
    db.commit()
    # print(newsId)
